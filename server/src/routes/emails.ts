...
/**
 * Inbound webhook handler. Ingests student email requests, routes them,
...
router.post('/inbound', async (req, res) => {
  ...
  try {
    ...
    if (ticket) {
      ...
      console.log(`[INBOUND WEBHOOK] Threading reply to existing Ticket #${ticket.ticketNumber}`);
      ...
      if (aiResult.confidence >= config.AUTO_REPLY_CONFIDENCE_THRESHOLD) {
        ...
        console.log(`[INBOUND WEBHOOK] Automatically sent high-confidence AI response for Ticket #${ticket.ticketNumber}`);
      }
    } else {
      // Message is a new inquiry. Create a new Ticket.
      console.log(`[INBOUND WEBHOOK] Creating a new ticket for student: ${studentEmail}`);
